...
@RequiresApi(Build.VERSION_CODES.O)
fun displayChangeLanguageAlert(context : Context, activity: Activity) {
    val listLang = arrayOf("English", "Thai")
    val font: Typeface = ResourcesCompat.getFont(context, R.font.prompt_bold) as Typeface
    val textView = TextView(context)
    val next = "<font color='#EE0000'>${context.getString(R.string.warning)}</font>"
    textView.text = Html.fromHtml("$next<br>${context.getString(R.string.Select_Language)}")
    textView.typeface = font
    textView.setTextColor(context.getColor(R.color.black))
    textView.textSize = 20F
    textView.setPadding(50,50,0,0);
